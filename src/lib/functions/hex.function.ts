...
export function fnHex(ctx: Context, parms: (TExprStackItem | undefined)[]) {
	const parm = parms[0];
	if (!parm || parm.type !== TOKEN_TYPES.NUMBER)
		throw new VAParseError(`HEX: Parameter should be a number  - "${parm}"`);
