...
export function fnIif(ctx: Context, parms: (TExprStackItem | undefined)[]) {
	const parm = parms[0];
	if (parm?.type !== TOKEN_TYPES.NUMBER)
		throw new VAParseError(`IIF: Parameter should be a number =0 or !=0  - "${parm}"`);
