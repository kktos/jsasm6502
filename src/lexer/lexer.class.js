...
export class Lexer {
	...
	addEventListener(type, listener) {
		if (typeof listener !== "function")
			throw new TypeError(
				"FATAL ERROR: Lexer.addEventListener needs a function",
			);

		if (!this.ctx.eventHandlers[type]) this.ctx.eventHandlers[type] = [];
	...
	removeEventListener(type, listener) {
		if (!this.ctx.eventHandlers[type]) return;
		const idx = this.ctx.eventHandlers[type].indexOf(listener);

		// console.log("Lexer.removeEventListener",type, listener);
		// console.log("Lexer.eventHandlers",this.ctx.eventHandlers);

		if (idx > -1) this.ctx.eventHandlers[type].splice(idx, 1);
	}
	executeEventListener(type) {
		for (const handler of this.ctx.eventHandlers[type]) handler();
	}
	...
	lookahead(idx = 1) {
		if (idx < 0) {
			if (this.ctx.tokCount - idx < 0) return false;
			return this.ctx.tokens[this.ctx.tokCount - idx];
		}

		if (this.ctx.curTokIdx >= this.ctx.tokCount - idx) return false;
	...
	isToken(tokenType) {
		return this.ctx.curTokIdx < this.ctx.tokCount
			? tokenType === this.ctx.tokens[this.ctx.curTokIdx].type
			: false;
	}
	...
	token() {
		if (this.ctx.curTokIdx >= this.ctx.tokCount) return false; //new Token(TOKEN_TYPES.EOF);
	...
	_testLookaheadChars(chars) {
		if (this.ctx.posInLine + chars.length >= this.ctx.currLine?.length)
			return false;
		let charIdx = 0;
		while (
			charIdx < chars.length &&
			chars[charIdx] ===
				this.ctx.currLine?.[this.ctx.posInLine + charIdx].toUpperCase()
		) {
			charIdx++;
		}
	...
	_testLookaheadChar(charset) {
		...
		return charset.has(
			this.ctx.currLine?.[this.ctx.posInLine].toUpperCase() ?? null,
		);
	}
	...
	_advance() {
		...
		if (IDENTIFIER_CHARSET.has(this.ctx.currChar.toUpperCase())) {
			...
			this.ctx.currToken.text = this.ctx.currLine.slice(
				startPos,
				this.ctx.posInLine,
			);
		...
		if (QUOTES_CHARSET.has(this.ctx.currChar)) {
			...
			this.ctx.currToken.value = this.ctx.currLine.slice(
				startPos + 1,
