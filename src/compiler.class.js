...
export class Compiler {
	...
	select(segmentName) {
		...
		this.pc =
			this.segments[this.currentSegment].start +
			this.obj[this.currentSegment].length;
	}
	...
	setPC(addr) {
		...
		if (addr < seg.start || addr > seg.end)
			throw new VABuildError(
				`ORG is out of Segment "${this.currentSegment}" range ${getHexWord(
					seg.start,
				)}:${getHexWord(seg.end)}`,
			);
		this.pc = addr;
	...
	emits(pass, bytes, wannaShowChars) {
		...
		if (this.pc + bytes.length > seg.end + 1)
			throw new VABuildError(
				`Code is out of Segment "${this.currentSegment}" range ${getHexWord(
					this.pc,
				)}:${getHexWord(this.pc + bytes.length)} > ${getHexWord(
					seg.start,
				)}:${getHexWord(seg.end)}`,
			);
	...
	dump(segmentName, bytePerLine = 16) {
		...
		for (let addr = codeStart - offset; addr < codeEnd; addr++) {
			...
			if (addr < codeStart) s += ".. ";
				...
				s +=
					addr % bytePerLine === bytePerLine - 1 || addr === codeEnd - 1
						? "\n"
						: " ";
			}
		}
