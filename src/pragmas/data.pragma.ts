...
export function pushNumber(list: number[], parm: TExprStackItemNumber, endianSize: number) {
	...
	switch (endianSize) {
		// byte
		case 1:
			if (parm.value > 0xff) throw new VAParseError("Data Overflow - 8bits expected");
			list.push(byte0);
			break;

		// word (2 bytes) little endian
		case 2:
			if (parm.value > 0xffff) throw new VAParseError("Data Overflow - 16bits expected");
	...
	while (true) {
		...
		switch (res?.type) {
			case TOKEN_TYPES.NUMBER:
				pushNumber(list, res as TExprStackItemNumber, endianSize);
				break;
